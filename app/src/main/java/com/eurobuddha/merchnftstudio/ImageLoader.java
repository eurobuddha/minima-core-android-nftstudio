...
public final class ImageLoader {
    ...
    /** Synchronous decode for the export pipeline (call OFF the UI thread). Same handling as the
     *  async loads: data:/http(s)/ipfs, SVG rasterise, downsample, null on any failure. */
    public static Bitmap decodeSync(String url, int reqPx) {
        if (url == null || url.isEmpty()) return null;
        return decode(url, reqPx);
    }
